# Slides Module — Product Requirements (Corrected)

## Context

This document defines the Slides module as a system that converts HTML into a structured, editable slide model and exports it across formats.

This version corrects prior issues:
- User stories are now outcome-driven, not vague
- Acceptance criteria are testable and specific
- Gap analysis is explicitly included

---

# Epic 1: HTML Import — Accurate Visual Reconstruction

## Intent
The system must reconstruct the *rendered visual output* of HTML, not the DOM structure.
...
## Implementation Requirements for Clean PDF/Print Export

The PDF export should not rely on users manually printing the editor page. The app should generate a clean export view from SlideDocument JSON.

Required behavior:
- Render an export-only HTML document from SlideDocument
- Exclude all editor UI and canvas helper styles
- Add print-safe CSS:
...
If the export pipeline uses browser automation, PDF settings should explicitly set:
- `printBackground: true`
- `displayHeaderFooter: false`
- zero margins
- page size matching the slide

Manual browser print fallback:
- Open exported HTML, not the editor page
- Use Save as PDF
...
### 1. Layout Reconstruction Failure
- Text collapsed to bottom-left
- Indicates coordinate system mismatch
- Root not correctly applied

### 2. Scaling Bug
- Font sizes not scaled with layout
- Coordinates transformed without size adjustments

### 3. Background Loss
- Slide background not extracted from `.page`
- Editor fallback color applied instead

### 4. Duplicate Layer Extraction
- Parent + child text imported
- Results in overlapping text blocks

### 5. DOM-Based Parsing Instead of Render-Based Parsing
- Flex/grid layouts not preserved
- Layout inferred incorrectly
...
- Canonical coordinate system enforcement
- Render-first parsing model
- Text extraction rules (leaf-only)
- Shape/container detection logic
...
- Fonts causing layout drift
- External CSS not loading
- Unsupported CSS silently failing
- Export mismatch due to inconsistent model

---

## Definition of Done

Import is only complete when:

- Slide visually matches original HTML
- No duplicate or overlapping text
- Layout structure preserved
- Background preserved
- Text scale correct
- User can immediately edit without fixing layout

---

## Core Principle

This system must never attempt to edit HTML directly.

All workflows must follow:

HTML → Render → Parse → SlideDocument → Edit → Export
