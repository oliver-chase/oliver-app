---
ID: US-O6
Title: Wishlist Item — Click Opens Edit Modal with Full Design Style
Status: Missing
Verified: false
Backdated: 2026-04-24
---

Current state:
- No Wishlist route or wishlist item entity flow found in current route/components tree.

Gap:
- Feature surface absent in this repo.

Backlog acceptance:
- Introduce wishlist item list + click-to-edit modal flow.
- Reuse canonical modal shell/components.
- Ensure pre-populated edit fields and save/update behavior.

