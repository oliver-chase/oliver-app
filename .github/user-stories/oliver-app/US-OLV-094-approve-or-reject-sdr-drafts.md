...
- [ ] Approve posts the selected item id and action approve to the backend.
- [ ] Reject posts the selected item id and action reject to the backend.
- [ ] Failed requests alert or display the returned error and preserve the item state.

Notes: Client calls /api/sdr-approve, but no matching function file was found in functions/api during inspection.
---
